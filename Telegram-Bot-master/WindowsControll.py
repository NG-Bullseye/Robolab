...
def  main():
    ...
    while True:
        ...
        while text == 'Choose Program':
            sleep(SLEEP_TIME)
            chat_id, text, update_id = get_last_id_text(get_updates(update_id + 1))
            #chat_id, text, update_id = get_last_id_text(get_updates(update_id + 1))
        #playerButtonAdding(tag)
        if text=="R6Info":
            #subprocess.call(["C:\\Ubisoft Game Launcher\\games\\Tom Clancy's Rainbow Six Siege\\RainbowSix.exe"])
            subprocess.call(["C:\\Users\\leona\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Startup\\R6Info.py"])
